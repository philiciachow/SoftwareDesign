...
while page < 200:
    ...
    for tag in cdc.find_all("ul", "results"):
        links.extend(tag.find_all("li")[0].find_all("li")[2].text.lower().replace("(", "").replace(",", "").replace("-", "").replace(".", "").replace(")", ""). replace(":", "").encode("utf-8").split()) # add the list of links we find for each a tag to the list: links
